...
## RULE-001：本專案架構的 AWS 資源部署一律使用 Terraform，禁止用 AWS CLI 部署

- **規則**：本專案架構下的 AWS 資源的建立/修改/刪除一律透過 Terraform（`infra/`）進行。禁止使用 `aws cli` 執行任何部署/變更性質指令（如 `aws s3 mb`、`aws s3api put-bucket-policy`、`aws glue create-database` 等）。
...
## RULE-003：本專案架構的 IAM 權限設定一律遵照 AWS 官方 Best Practice，衝突時才討論特例並留 ADR

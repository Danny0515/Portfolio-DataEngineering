...
- **Silver**（`silver.stock`，spec item 6）：負責去重（`(symbol, date)` unique）、型別校正（price → decimal、date → date type）、標準化欄位命名
- **Gold**（`gold.monthly_ohlcv`，spec item 7）：對外可查詢的月頻行情聚合寬表
...
- ⚠️ **注意**：Silver/Gold 的實際實作（spec item 6/7）與 partition 設計（item 9）記錄於 [ADR-0003](0003-append-vs-overwrite.md)，本 ADR 只記錄分層理由本身

...
> 狀態：**草稿 — §3 待確認事項待使用者拍板**，拍板後才展開 §4 實作清單細節。
...
| 選項 | 說明 | 適合情境 |
| --- | --- | --- |
| A. Great Expectations | 規則表達力強、社群大、可產生 Data Docs（HTML 品質報告），但設定較重 | 想要作品集展示「品質報告」這個可視化產出 |
| B. Soda Core | YAML 定義檢核規則，設定輕量、上手快 | 想把力氣留給 WAP 機制本身，品質規則越簡單越好 |

> 建議：A（Great Expectations），因為 plan.md §3 已將其列為主選，且 Data Docs 產出剛好呼應本 Slice「品質可視化」的展示目的。

### 3.2 WAP 暫存分支機制：Iceberg 原生 branch vs 手動 staging table

| 選項 | 說明 |
| --- | --- |
| A. Iceberg 原生 branch（`CREATE BRANCH` / `.branch_staging`） | 用 Iceberg table branching 功能，Audit 通過後 `fast-forward` 或 `cherry-pick` 到 main branch。更貼近 WAP pattern 的原始設計，但需先確認 Glue Catalog + 目前 Spark/Iceberg 版本組合是否完整支援 branch 操作 |
| B. 手動 staging table | 建一張 `silver_staging.stock`，Audit 通過後用 Spark 讀出重寫進 `silver.stock`。實作直觀、不依賴特定版本的 branch 功能，但少了「同一張表、不同分支」的優雅性 |

> 建議：先花小量時間 spike A（Iceberg branch），若目前 AWS Glue + Spark 版本組合不支援或操作卡關，降級用 B，並在 ADR 記錄原因（呼應 CLAUDE.md 的 RULE-003 精神：best practice 優先，卡關才討論特例）。

### 3.3 壞資料如何注入

| 選項 | 說明 |
| --- | --- |
| A. 擴充 Slice 0 的 generator | 在 `generate_stock_data.py` 加一個 `--inject-dirty` 開關，產生負值/空值/重複 (symbol, date) 等髒資料 | 
| B. 獨立注入 script | 另開一支 script，直接對已落地的 raw CSV 做污染，不動 Slice 0 既有 generator |

> 建議：A，因為 Slice 0 §8 風險已經預留了這個接口（"需在 generator 中預留『注入髒資料』的開關"），沿用可維持單一資料產生入口。

### 3.4 Data Contract 範圍

| 選項 | 說明 |
| --- | --- |
| A. 只涵蓋 Silver 層 `stock` | 契約只約束「進入 Silver 前必須滿足什麼」，對應本 Slice 品質關卡的實際作用位置 |
| B. 同時涵蓋 Silver 與 Gold 層 | 契約也約束 Gold 層 `monthly_ohlcv` 的 schema/SLA |

> 建議：A，契約範圍跟品質關卡的作用位置對齊（Bronze→Silver 之間），Gold 層契約留待有更多下游 consumer 時（例如 Slice 3 Serving）再擴充，避免這個 Slice 顧太多層。

---

## 4. 實作項目清單 (Implementation Checklist)

> 待 §3 四題拍板後，此清單即為實作依據。項目大致依序執行。
...
| 7 | Data Contract 撰寫 | 依 §3.4 範圍，定義 schema/品質規則/SLA | `docs/contracts/market-data.contract.yaml` | |
...
| `docs/contracts/market-data.contract.yaml` | 第一份生效的 Data Contract | 納入 Git 版控 |
...
| 一致性 (Consistency) | `symbol` 需為 §3.4 資料範圍內定義的合法代號（沿用 Slice 0 的 `2330`/`2454`/`3653`），不接受未知代號 |
...
- [ ] `docs/contracts/market-data.contract.yaml` 納入版控，且內容與 §6 規則、Silver schema 一致
- [ ] 上述 §9 文件皆已產出
...
- **相依**：Slice 0 的 Silver（[slice0-batch-market-data.md](slice0-batch-market-data.md) 項目 6）與 Gold（項目 7）需先完成，本 Slice 才有正式 Silver 可以疊加關卡
- **相依**：§3 四題待確認事項需先拍板，才能展開 §4 實作清單細節
- **風險**：§3.2 若選 Iceberg 原生 branch，需先確認目前 AWS Glue Catalog + Spark/Iceberg 版本組合是否完整支援 branch 操作，卡關需降級為手動 staging table 並留 ADR 記錄
- **風險**：Great Expectations（若選 §3.1 選項 A）在 Spark on Glue 環境的整合複雜度未知，需預留 spike 時間
...
- Decision Log：Great Expectations vs Soda Core 的取捨（§3.1 拍板後補正式理由）
